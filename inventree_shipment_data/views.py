...
def calculate_shipment_data_for_shipment(shipment: SalesOrderShipment):
    """Calculate weight/volume totals for a specific shipment.

    Similar to calculate_shipment_data but iterates over allocations attached
    to the shipment rather than the order lines. Returns same dict structure.
    """
    parts = []
    total_weight = 0.0
    total_volume = 0.0
    ...
    # iterate over allocations related to this shipment
    for alloc in shipment.allocations.select_related("line__part").all():
        part = alloc.line.part if alloc.line else None
        qty = float(alloc.quantity or 0)
...
class ShipmentShipmentView(APIView):
    """Return weight/volume information for a specific shipment."""

    permission_classes = [permissions.IsAuthenticated]
    serializer_class = SalesOrderShipmentSerializer

    def get(self, request, pk, *args, **kwargs):
        shipment = get_object_or_404(SalesOrderShipment, pk=pk)
        data = calculate_shipment_data_for_shipment(shipment)
